...
export default async function handler(req, res) {
 ...
 if(req.method === "POST")
 {
  const { name, courseName,date,certificateType,id,durationStartDate,durationEndDate} = req.body
  console.log(name+"name", courseName+"courseName",date+"date",certificateType+"certificateType",id+"id",durationStartDate+"durationStartDate",durationEndDate+"durationEndDate");
  let path=""
 ...
 const { sheets } = await authentication();
  ...
  try {
    ...
    const html = template({
      name, courseName,date,certificateType,id,durationStartDate,durationEndDate
    });
 ...
 // set our compiled html template as the pages content
    ...
    const pdf = await page.pdf({width:"1122", height:"793",printBackground:true});
